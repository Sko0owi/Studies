...
Buffer<pair<int,int>,MAX_EDGE> candidatesToPickCopy; 
...
void generatePath()
{
    ...
    for(int i = 0 ; i < numberOfCells; i++)
    {
        ...
        if(cells[i].resourceType != kEmpty) resourceCells++;
    }
            ...
            for(int i = 0; i < weights.Size(); i++)
            {
                ...
                if(sum > rand) 
                {
                    ...
                    {   
                        ...
                        alreadyInPath[candidatesToPick[i].second] = 1;
                        ...
                        for(int cell = 0 ; cell < numberOfCells; cell++)
                        {
                            if(cells[cell].resourceType != kEmpty && cells[cell].resourceCount != 0 && alreadyInPath[cell] == 0) candidatesToPick.Add({candidatesToPick[i].second, cell});
                        }

                        candidatesToPickCopy.Reset();

                        for(int j = 0; j < candidatesToPick.Size(); j++)
                        {
                            if(candidatesToPick[j].second != candidatesToPick[i].second)
                            {
                                candidatesToPickCopy.Add(candidatesToPick[j]);
                            }
                        }

                        candidatesToPick.Reset();

                        for(auto candidate : candidatesToPickCopy)
                        {
                            candidatesToPick.Add(candidate);
                        }
