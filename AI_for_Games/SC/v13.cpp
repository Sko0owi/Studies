...
int timeForChoosing = 700;
int timeForUpgrading = 180;
...
int minDistFromEnemy[MAX_N], minDistFromMe[MAX_N];
...
double generateWeight(int from, int to)
{
    ...
    return (double)(0.5 / (double) (shortestDist[from][to] + abs(minDistFromEnemy[to] - minDistFromMe[to]) + 1));
}
...
pair<int,int> getScore()
{
    ...
    while(idx != 0)
    {
        ...
        if(cells[top].resourceType == kEggs)
        {
            ...
            double weightEggs = 5.0;

            if(earlyGame) weightEggs = 10.0;
            
            weightEggs *= 1.0 / (double) (abs(minDistFromMe[top] - minDistFromEnemy[top]) + 1);

            weightEggs = max(1.0, weightEggs);

            int extracted = min(cells[top].chainMe, cells[top].resourceCount) * weightEggs;


            harvestedEggs += min(cells[top].chainMe, cells[top].resourceCount);
            capturedResources.second += extracted;
    ...
    for(auto base : myBase)
        idx = max(idx,cells[base].myAnts + harvestedEggs);
    ...
    for(auto base : myBase)
    {
        queueForDial[cells[base].myAnts + harvestedEggs].Add(base);
        cells[base].chainMe = cells[base].myAnts + harvestedEggs;
    }
    ...
    while(idx != 0)
    {
        ...
        if(cells[top].resourceType == kCrystal)
        {
            ...
            if(earlyGame) weightCrytal = 1.0;

            weightCrytal *= 1.0 / (double) (abs(minDistFromMe[top] - minDistFromEnemy[top]) + 1);
...
{   
    ...
    double weightEggs = myBase.Size() == 2 ? 4 : 2;
    double weightCrystal = 1;


    if(earlyGame)
    {
        weightEggs = myBase.Size() == 2 ? 10 : 5;
        weightCrystal = 0.25;
    } 
    ...
    int collectedEval = collectedResources.first * weightCrystal + collectedResources.second * weightEggs;
...
void prepare()
{
    ...
    {
        minDistFromMe[i] = 1e9;
        minDistFromEnemy[i] = 1e9;

        for(auto base : myBase)
            minDistFromMe[i] = min(shortestDist[base][i], minDistFromMe[i]);

        for(auto base : enemyBase)
            minDistFromEnemy[i] = min(shortestDist[base][i], minDistFromEnemy[i]);
    }
...
int main()
{
    ...
    while(true)
    {
        ...
        earlyGame = (100 * (myScore / allCrystal) < 10) && 100 * ((enemyScore / allCrystal) < 10);
