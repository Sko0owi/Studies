...
int timeForChoosing = 80;
int timeForUpgrading = 15;
...
Buffer<pair<int,int>,MAX_EDGE> bestPathCopy; 
...
pair<int,int> getScore(int debug)
{
    ...
    {
        auto top = kol.top(); kol.pop();
        if(cells[top.second].resourceType == kEggs)
        {
            capturedResources.second += min(cells[top.second].chainMe, cells[top.second].resourceCount);
            if(debug) cells[top.second].resourceCount -= min(cells[top.second].chainMe, cells[top.second].resourceCount);
        }
        for (auto neigbour: cells[top.second].neighbours)
        {
...
{   
    ...
    for(int turn = 0 ; turn < turns; turn++)
    {
        ...
        pair<int,int> afterTurnScore = getScore(true);
...
void upgradePath()
{
    ...
    while (chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - startTime).count() <= timeForUpgrading)
    {
...
int main()
{
    ...
    while(true)
    {
        ...
        for(int i = 0; i < numberOfCells; i++)
        {
            ...
            cellsCopy[i].Update(resourcesCount, myAnts, enemyAnts);
            cells[i].Update(cells[i].resourceCount, myAnts, enemyAnts);
        ...
        // for(int i = 0; i < numberOfCells; i++)
        // {
        //     cerr << i << " " << translate(cells[i].resourceType) << " " << cells[i].resourceCount << " " << cells[i].myAnts << "\n";
        // }

        // pair<int,int> digged = getScore(0);

        // cerr << myScore << " " << currAnts << "\n";

        // cerr << digged.first << " " << digged.second << "\n";

        // cerr << prevScore << " " << prevAnts << "\n";
        ...
        if(prevAnts != -1)
        {
            //assert(prevAnts + digged.second == currAnts);
        }
        ...
        if(prevScore != -1)
        {
            //assert(prevScore + digged.first == myScore);
        }
        ...
        cout << "MESSAGE T: " << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - startTime).count() << " S: " << simulations;
        cout << endl;
