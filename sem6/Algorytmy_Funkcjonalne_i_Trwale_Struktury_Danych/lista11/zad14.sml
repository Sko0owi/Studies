...
fun uncons (Two(Leaf x , Leaf y) :: ts) = (x, One(Leaf y) :: ts)
    | uncons ([One(Leaf x)]) = (x, empty)
    | uncons (One(Leaf x) :: ts) = let val (Node(w,t1,t2),ts2) = uncons ts in 
        (x, Two(t1,t2) :: ts2) end
    | uncons ([]) = raise Empty

fun tail ts = #2(uncons ts)
...
fun lookup (i, []) = raise Subscript
    ...
    | lookup (i, Two (t1, t2) :: ts) = 
        ...
        else if i <= size t2 + size t1 then lookupTree(i - size t1, t2) 
