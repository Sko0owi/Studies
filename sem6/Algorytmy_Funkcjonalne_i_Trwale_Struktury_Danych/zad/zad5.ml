...
module Queue (Ord : OrderedType) (HM : T with type elt := Ord.t) :
 Q with type elt := Ord.t = struct
         type elt = Ord.t
         type t = elt list * HM.t

         let empty = [],HM.empty
         let isEmpty (f,q) = match f with
         | [] -> HM.isEmpty q
         | _ -> false

         let cons x (f,q) = (x :: f, q)
         let snoc x (f,q) = (f, HM.snoc x q)
         let tail (f,q) =
                 match f with
                 | [] -> ([], HM.tail q)
                 | x :: xs -> (xs, q)
...
module QI = Queue(Ordered_Ints)(HMI)
